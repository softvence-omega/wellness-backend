...
//   }
  ...
  async update(
    ...
    if (updateMedicalReportDto.reportDate && updateMedicalReportDto.reportDate > new Date()) {
      throw new BadRequestException('Report date cannot be in the future');
    }
  ...
  private validateReportData(reportData: ReportData): void {
    if (!reportData.patient_name || !reportData.report_date || !reportData.lab_values) {
      throw new BadRequestException('Report data must include patient_name, report_date, and lab_values');
    }

    if (!Array.isArray(reportData.lab_values) || reportData.lab_values.length === 0) {
    ...
    reportData.lab_values.forEach((labValue: LabValue, index: number) => {
      if (!labValue.test_name || labValue.value === undefined || !labValue.unit || !labValue.ref_ranges || !labValue.status) {
        throw new BadRequestException(`Lab value at index ${index} is missing required fields`);
      }

      if (typeof labValue.value !== 'number' || isNaN(labValue.value)) {
        throw new BadRequestException(`Lab value at index ${index} must be a valid number`);
      }

      const validStatuses = ['within', 'below', 'above', 'borderline'];
      if (!validStatuses.includes(labValue.status)) {
        throw new BadRequestException(`Lab value at index ${index} has invalid status. Must be one of: ${validStatuses.join(', ')}`);
      }
    });
  }

  // Additional method to get abnormal lab values
  async getAbnormalLabValues(reportId: string, userId: string): Promise<LabValue[]> {
  ...
  async getReportsWithCriticalValues(userId: string): Promise<IMedicalReport[]> {
    try {
      ...
      return allReports.reports.filter(report => {
        ...
        return reportData.lab_values && reportData.lab_values.some(
          (labValue: LabValue) => labValue.status === 'below' || labValue.status === 'above'
        );
      });
    } catch (error) {
      throw new InternalServerErrorException(`Failed to get critical reports: ${error.message}`);
    }
  }

  // Optional: Method to get reports with borderline values
  async getReportsWithBorderlineValues(userId: string): Promise<IMedicalReport[]> {
    try {
      const allReports = await this.repository.findAll({ 
        userId, 
        limit: 1000,
      ...
      throw new InternalServerErrorException(`Failed to get borderline reports: ${error.message}`);
    }
  }
  ...
  async getHealthSummary(userId: string): Promise<{
    ...
    try {
      ...
      throw new InternalServerErrorException(`Failed to get health summary: ${error.message}`);
    }
  }
}