...
export class NudgesController {
  ...
  create(@Request() req, @Body() createNudgeDto: CreateNudgeDto) {
    ...
    return this.nudgesService.create(userId, createNudgeDto);
  }

  // NOTIFICATIONS - SEPARATE ROUTE
  @Post('notifications')
  @ApiOperation({ summary: 'Set notification preferences' })
  setNotifications(@Request() req, @Body() dto: SetNotificationsDto) {
    const userId = req.user?.userId || req.user?.id || req.user?.sub;
    if (!userId) {
      throw new BadRequestException('User ID is required');
    }
  ...
  getTodayProgress(@Request() req) {
    const userId = req.user?.userId || req.user?.id || req.user?.sub;
    return this.nudgesService.getTodayProgress(userId);
  }

  @Get('stats')
  getStats(@Request() req) {
    const userId = req.user?.userId || req.user?.id || req.user?.sub;
    return this.nudgesService.getStats(userId);
  }

  @Get(':id')
  findOne(@Request() req, @Param('id') id: string) {
    const userId = req.user?.userId || req.user?.id || req.user?.sub;
    return this.nudgesService.findOne(id, userId);
  }

  @Patch(':id')
  update(@Request() req, @Param('id') id: string, @Body() dto: UpdateNudgeDto) {
    const userId = req.user?.userId || req.user?.id || req.user?.sub;
    return this.nudgesService.update(id, userId, dto);
  }
  ...
  updateProgress(
    @Request() req,
    @Param('id') id: string,
    @Body() dto: UpdateNudgeProgressDto,
  ) {
    const userId = req.user?.userId || req.user?.id || req.user?.sub;
    return this.nudgesService.updateProgress(id, userId, dto);
  }
  ...
  remove(@Request() req, @Param('id') id: string) {
    const userId = req.user?.userId || req.user?.id || req.user?.sub;
    return this.nudgesService.remove(id, userId);
  }
}
